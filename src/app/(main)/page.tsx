...
import { env } from "~/env";
// helper to build a Cloudinary thumbnail URL from public_id
function cloudinaryThumb(publicId: string) {
  return `https://res.cloudinary.com/${env.CLOUDINARY_CLOUD_NAME}/image/upload/f_auto,q_auto,w_300,h_300,c_fill/${publicId}`;
}
export default async function Page() {
  ...
  const songsWithUrls = songs.map((song) => {
    // Prefer direct Cloudinary URL if saved; else build from public_id
    const s = song as {
      coverImageUrl?: string | null;
      coverImagePublicId?: string | null;
    };
    const thumbnailUrl =
      s.coverImageUrl ?? (s.coverImagePublicId ? cloudinaryThumb(s.coverImagePublicId) : null);
  ...
  return (
    <div className="p-4">
      <h1 className="text-3xl font-bold tracking-tight">Discover Music Here</h1>
